...
public class Example4LexerTest{
	...
	public void Example4LexerTestTest() throws Exception {
		var lexer = new Lexer(
			"class Example4\n"+
			"\tnumber a\n"+ // should be neline, dendent, newlien
			...
			"    shared helloWorld()  { no parameters, no return value, public} \n"+
					//count spaces to get indent for shared
		...
		Assertions.assertEquals(Token.TokenTypes.DEDENT, tokens.get(7).getType()); // actual NEWLINE , gets skipped
		Assertions.assertEquals(Token.TokenTypes.NEWLINE, tokens.get(8).getType());
		Assertions.assertEquals(Token.TokenTypes.INDENT, tokens.get(9).getType()); //actuall shared, not counting spaces for indent
